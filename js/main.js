const Game = {
    ...
    loop() {
        ...
        if (PickupManager.update(dt, this.player ? this.player.group.position : null)) {
            this.ammo = Math.min(99, this.ammo + 10);
    ...
    handleInput(dt) {
        ...
        if (p.playerRing) {
            ...
            p.playerRing.material.opacity = pulse;
        }

        p.fireTimer -= dt;
        if ((InputManager.mouseDown || InputManager.isKeyDown('Space')) && p.canFire() && this.ammo > 0) {
            this.playerFire();
            p.resetFireTimer();
        }
    },
    ...
    playerFire() {
        const p = this.player;
        const tip = p.getBarrelTip();
        const dir = p.getBarrelDirection();
        BulletManager.fire(GameScene.scene, tip, dir, true);
        this.ammo--;
        UI.updateAmmo(this.ammo);
    },
    ...
    updateEnemies(dt) {
        ...
        this.enemies.forEach(enemy => {
            ...
            if (enemy.canFire() && enemy.aiState === 'chase') {
                const tip = enemy.getBarrelTip();
                const dir = enemy.getBarrelDirection();
                const toPlayer = new THREE.Vector3().subVectors(playerPos, enemy.group.position);
                if (toPlayer.length() < 40) {
                    BulletManager.fire(GameScene.scene, tip, dir, false);
    ...
    checkCollisions() {
        ...
        for (let i = bullets.length - 1; i >= 0; i--) {
            ...
            if (b.isPlayer) {
                this.checkPlayerBullet(b, bPos);
            } else {
                this.checkEnemyBullet(b, bPos);
            }
        }
    },
    ...
    checkBulletVsEnvironment(b, bPos) {
        for (const barrel of GameScene.barrels) {
            ...
            if (bPos.distanceTo(barrel.position) < 1.5) {
                b.dispose();
        ...
        for (const building of GameScene.buildings) {
            ...
            if (bPos.distanceTo(building.position) < building.radius) {
                ...
                if (building.health <= 0) {
                    ExplosionManager.create(GameScene.scene, building.position.clone(), true);
                    this.triggerShake(0.4, 0.3);
                    GameScene.removeDestructible(building);
                }
                return;
            }
        }
        ...
        for (const bunker of GameScene.bunkers) {
            ...
            if (bPos.distanceTo(bunker.position) < bunker.radius) {
                ...
                ExplosionManager.create(GameScene.scene, bPos.clone(), false);
                if (bunker.health <= 0) {
                    ExplosionManager.create(GameScene.scene, bunker.position.clone(), true);
                    this.triggerShake(0.3, 0.2);
                    GameScene.removeDestructible(bunker);
                }
                return;
            }
        }
    ...
    explodeBarrel(barrel) {
        ...
        setTimeout(() => {
            ...
            for (const other of GameScene.barrels) {
                ...
                if (barrel.position.distanceTo(other.position) < 6) {
                    this.explodeBarrel(other);
                }
            }
        }, 100);
    },
    ...
    damageNearby(pos, radius, damage) {
        if (this.player && this.player.alive && !this.invulnerable) {
            if (pos.distanceTo(this.player.group.position) < radius) {
        ...
        this.enemies.forEach(e => {
            if (e.alive && pos.distanceTo(e.group.position) < radius) {
                e.takeDamage(damage);
            }
        });

        SoldierManager.soldiers.forEach(s => {
            if (s.alive && pos.distanceTo(s.group.position) < radius) {
                s.takeDamage(damage);
            }
        });
    },
    ...
    updateCamera(dt) {
        ...
        const lerpSpeed = this.aiming ? 8 : 5;
        GameScene.camera.position.lerp(targetCamPos, lerpSpeed * dt);

        if (this.screenShake.timer > 0) {
            this.screenShake.timer -= dt;
            const ratio = this.screenShake.timer / this.screenShake.duration;
            const shakeAmt = this.screenShake.intensity * ratio;
        ...
        const forward = new THREE.Vector3(0, 0, -10).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.playerYaw);
        lookAt.add(forward);
...
function restartGame() {
    ...
    const toRemove = [];
    GameScene.scene.traverse(child => {
        if (child !== GameScene.scene) toRemove.push(child);
    });
